...
public class RulerYearRendererImpl implements ChronographStageRulerRenderer {
	...
	public void draw(GC gc, Rectangle bounds, int scale, int width, int tiksOffset, int xAxis) {
		...
		int yBottomPosition = bounds.y + bounds.height - RullerStyler.RULER_YEAR_HEIGHT;
		int xPosition = 0;
		calendar.clear();
		...
		gc.fillGradientRectangle(xPosition, yBottomPosition, bounds.width, RullerStyler.RULER_YEAR_HEIGHT, true);
		while (true) {
			if (calendar.get(Calendar.DAY_OF_MONTH) == 1) {
				gc.setForeground(RullerStyler.RULER_BRD_COLOR);
				gc.drawLine(xPosition, yBottomPosition + RullerStyler.RULER_YEAR_HEIGHT, xPosition, yBottomPosition);
