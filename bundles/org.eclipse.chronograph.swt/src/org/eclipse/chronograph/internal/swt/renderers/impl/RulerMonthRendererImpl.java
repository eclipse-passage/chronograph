...
public class RulerMonthRendererImpl implements ChronographStageRulerRenderer {
	...
	public void draw(GC gc, Rectangle bounds, int scale, int width, int tiksOffset, int xAxis) {
		...
		gc.fillRectangle(x, yBottomPosition, bounds.width, RulerStyler.RULER_MOUNTH_HEIGHT);
		while (true) {
			...
			if (calendar.get(Calendar.DAY_OF_MONTH) == 1) {
				...
				gc.drawString(msg, x + 4, yBottomPosition + 3, true);
				gc.drawLine(x, yBottomPosition, x, yBottomPosition + RulerStyler.RULER_MOUNTH_HEIGHT);
